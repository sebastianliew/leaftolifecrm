...
$prodMongoId = ($cr.body | ConvertFrom-Json)._id
Write-Host "Created product: $prodMongoId (HTTP $($cr.code))"
...
$er = Req -Method PUT -Url "$BASE/inventory/products/$prodMongoId" -Body $enableBody
Write-Host "PUT canSellLoose -> $($er.code): $($er.body.Substring(0,[Math]::Min(300,$er.body.Length)))"
...
$pr = Req -Method POST -Url "$BASE/inventory/products/$prodMongoId/pool" -Body $poolBody
Write-Host "POST /inventory/products/:id/pool (open) -> $($pr.code): $($pr.body)"

# Cleanup
$delBody = (@{ productIds=@($prodMongoId) } | ConvertTo-Json -Compress)
$dr = Req -Method POST -Url "$BASE/inventory/products/bulk-delete" -Body $delBody
Write-Host "Cleanup bulk-delete -> $($dr.code)"

...
export function useInventory() {
  ...
  const getProducts = useCallback(async (includeInactive = false) => {
    ...
    try {
      ...
      const params = includeInactive ? { limit: 1000, includeInactive: true } : { limit: 1000 };
      const response = await api.get('/inventory/products', params)
      if (!response.ok) {
        console.error('Failed to fetch products:', response.error)
        throw new Error(response.error || 'Failed to fetch products')
      }
      const data = response.data as { products?: Product[]; pagination?: { totalCount: number } } | Product[]
  ...
  const createProduct = useCallback(async (data: ProductFormData) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error('useInventory: API error response:', errorMessage);
  ...
  const adjustStock = useCallback(async (productId: string, quantity: number, _reason: string) => {
    ...
    try {
      ...
      const newCurrentStock = Math.max(0, (product as Product).currentStock + quantity)
  ...
  const getProductTemplates = useCallback(async (search?: string, page?: number, limit?: number) => {
    ...
    try {
      ...
      const response = await api.get('/inventory/products/templates', params)
      if (!response.ok) {
        throw new Error(response.error || 'Failed to fetch product templates')
      }
      const data = response.data as { products?: Product[]; pagination?: { totalCount: number } }
      setError(null)
      return data
    } catch (err) {
  ...
  const addStockToProduct = useCallback(async (data: StockAdditionData) => {
    setLoading(true)
    try {
      const response = await api.post('/inventory/products/add-stock', data)
      if (!response.ok) {

...
export default function ItemSalesReport() {
  ...
  const [serverSummary, setServerSummary] = useState<{ totalRevenue: number; totalCost: number; totalProfit: number; totalItems: number } | null>(null)
    ...
    const fetchData = async () => {
      try {
        ...
        const getDateRange = () => {
          ...
          switch (dateFilter) {
            case "24h":
              return { startDate: startOfDay(subDays(referenceDate, 1)), endDate: endOfDay(referenceDate) }
            case "7d":
              return { startDate: startOfDay(subDays(referenceDate, 7)), endDate: endOfDay(referenceDate) }
            case "14d":
              return { startDate: startOfDay(subDays(referenceDate, 14)), endDate: endOfDay(referenceDate) }
            case "28d":
              return { startDate: startOfDay(subDays(referenceDate, 28)), endDate: endOfDay(referenceDate) }
            case "all":
              return null
            case "custom":
  ...
  const exportCSV = () => {
    const headers = [
      "Item Name", "Total Sales", "Total Cost", "Total Discount", "Total Tax",
      "Avg List Price", "Avg Cost Price", "Quantity Sold", "Unit", "Margin %", "Last Sale Date",
    ]
    const rows = filteredData.map((item) => [
      `"${item.item_name.replace(/"/g, '""')}"`,
      item.total_sales.toFixed(2), item.total_cost.toFixed(2),
      item.total_discount.toFixed(2), item.total_tax.toFixed(2),
      item.average_list_price.toFixed(2), item.average_cost_price.toFixed(2),
  ...
  return (
    <div className="p-6 space-y-6">
      {/* Header — matches inventory & transactions pages */}
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">Item Sales Report</h1>
          <p className="text-sm text-gray-600">Detailed analysis of item sales performance</p>
        </div>
        <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="outline" className={cn("w-32 justify-start text-left font-normal", !customStartDate && "text-muted-foreground")}>
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="outline" className={cn("w-32 justify-start text-left font-normal", !customEndDate && "text-muted-foreground")}>
          ...
          {/* Export */}
          <Button variant="outline" onClick={exportCSV} disabled={filteredData.length === 0} className="flex items-center gap-2">
      ...
      <div className="flex items-center gap-6 text-sm text-gray-600">
        <span>Revenue: <strong className="text-gray-900">{formatCurrency(summary.totalRevenue)}</strong></span>
        <span>Cost: <strong className="text-gray-900">{formatCurrency(summary.totalCost)}</strong></span>
        <span>Profit: <strong className="text-gray-900">{formatCurrency(summary.totalProfit)}</strong></span>
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={13} className="text-center py-8 text-red-600">Error: {error}</TableCell>
                </TableRow>
              ) : data.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={13} className="text-center py-8">No data available</TableCell>
                </TableRow>
              ) : (
                filteredData.map((item, index) => {
                  ...
                  return (
                    <TableRow key={index}>
                      ...
                      <TableCell className="text-sm text-gray-600">{item.display_sku || '-'}</TableCell>
                      <TableCell>
                        {item.sale_type_label === 'Loose' ? (
                          <span className="px-1.5 py-0.5 rounded text-xs bg-amber-100 text-amber-800">Loose</span>
                        ) : (
                          <span className="px-1.5 py-0.5 rounded text-xs bg-blue-100 text-blue-800">{item.sale_type_label || 'Container'}</span>
                        )}
                      </TableCell>
                      ...
                      <TableCell className="text-right">
                        {item.margin === -1 ? (
                          <span className="text-orange-600" title="Cost data missing">N/A</span>
                        ) : (
                      ...
                      <TableCell className="text-right whitespace-nowrap">{date}</TableCell>
